...
function fn_save() {
	...
	//저장버튼 클릭
		...
		$.ajax({
			url: contextPath + "/nightCheckMngt/search/ajax/add/checkAll.do",
			type: "POST",
			dataType: "json",
			data: {
				addCheckAllList: JSON.stringify(updateDatas)
			},
...
function checkWay(row){
   ...
   $('#memo1').val(tableRef.jqGrid('getCell', row, 'DTL1'));   
   $('#CONFIRM').val(tableRef.jqGrid('getCell', row, 'CONFIRM'));   
...
function initTable () {
	...
	tableRef.jqGrid({
		...
		colNames: [ "점검대상여부","체크", "시스템","업무명", "구분","점검날자1","점검날자2","점검시간", "점검내용", "메모","A","B","C","D","점검결과","점검특이사안","점검시간","점검하기" ],
		colModel : [{
			width: "160px",
			id: "CHECK_YN",
			name: "CHECK_YN",
